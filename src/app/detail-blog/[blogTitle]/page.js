...
const BlogDetail = () => {
  ...
  useEffect(() => {
    if (blogs && blogTitle) {
      const foundBlog = blogs.find(
        (b) => b.title.replace(/\s+/g, "").toLowerCase() === blogTitle
      );
      setBlog(foundBlog);
    }
  }, [blogs, blogTitle]);
  ...
  const deleteBlog = async () => {
    try {
      const res = await axios.delete(`http://localhost:8000/blogs/${title}`);
  ...
  if (loading)
    return <p className="mt-5 text-3xl container mx-auto">Loading...</p>;
  if (error)
    return (
      <p className="mt-5 text-3xl font-bold container mx-auto text-red-600">
        Not Found
      </p>
    );
  return (
